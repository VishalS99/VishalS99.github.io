...
function TechBubble(props) {
  return (
    <div class="md:p-4 p-2 md:text-lg text-sm text-gray-300 badge badge-primary bg-transparent hover:badge-secondary hover:font-extrabold">
      {props.item}
    </div>
  );
}
      ...
      <div class="md:grid md:grid-cols-12 flex flex-col justify-center">
        <div class="md:col-span-3 ">
          <figure class="md:w-1/2 w-1/3 m-auto">
            <img class="w-full" src={props.logo} alt={props.company} />
          </figure>
        </div>
      ...
      <div class="md:text-xl w-full text-justify text-sm mt-2">
        <ul>
          <For each={props.desc.split("*")}>
            {(descItem) => <li class="m-4 font-normal">&#8594; {descItem}</li>}
          </For>
        </ul>
      </div>
  ...
  createRenderEffect(() => {
    fetch("/data.json")
      ...
      .then((data) => {
        ...
        setCurrentExperience(data.experiences[0]["data"]);
      });
  });
